...
class RateLimitMiddleware(BaseMiddleware):
    ...
    def _clean_old_violations(self, user_id: int) -> int:
        ...
        bucket['violation_timestamps'] = [
            ts for ts in bucket['violation_timestamps']
            if ts > cutoff_time
        ]
        ...
        if bucket.get('banned_until') and bucket['banned_until'] > now:
            ...
            if should_send_warning:
                ...
                if isinstance(event, Message):
                    ...
                    await event.answer(
                        f"🚫 Вы заблокированы за спам на {remaining_ban // 60} мин",
                        show_alert=True
                    )
                ...
                logger.debug(
                    f"🚫 BANNED user {user_id} blocked silently (warning cooldown)"
                )
