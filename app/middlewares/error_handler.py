...
async def global_error_handler(event: ErrorEvent) -> bool:
    ...
    if event.update.message:
        ...
        error_details["chat_type"] = event.update.callback_query.message.chat.type if event.update.callback_query.message else "unknown"
    ...
    if event.update.message:
        ...
        try:
            await event.update.callback_query.answer(
                f"❌ Произошла ошибка: {error_details['exception_type']}", 
                show_alert=True
            )
        except Exception as e:
