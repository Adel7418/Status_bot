    ...
    try:
        ...
        if order.status != OrderStatus.CLOSED:
            await message.answer(
                "❌ Редактировать можно только закрытые заявки (CLOSED). Введите другой ID:"
            )
            return
    finally:
        await db.disconnect()
...
async def send_edit_closed_menu(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Используем значения из FSM, если они есть, иначе из БД
        total = data.get("total_amount", order.total_amount)
        materials = data.get("materials_cost", order.materials_cost)
        has_review = data.get("has_review", order.has_review)
        out_of_city = data.get("out_of_city", order.out_of_city)
        ...
        text = (
            f"✏️ <b>Редактирование закрытой заявки #{order_id}</b>\n\n"
            f"<b>Текущие значения:</b>\n"
            f"• Общая сумма: <b>{total or 0.0:.2f} ₽</b>\n"
            f"• Расход: <b>{materials or 0.0:.2f} ₽</b>\n"
            f"• Чистая прибыль: <b>{net_profit:.2f} ₽</b>\n"
            f"• Прибыль мастера: <b>{master_profit:.2f} ₽</b>\n"
            f"• Прибыль компании: <b>{company_profit:.2f} ₽</b>\n"
            f"• Отзыв: {'✅ Да' if has_review else '❌ Нет'}\n"
            f"• Выезд за город: {'✅ Да' if out_of_city else '❌ Нет'}\n\n"
            f"Выберите поле для редактирования или сохраните изменения:"
        )
        ...
        kb.row(
            InlineKeyboardButton(
                text="💰 Общая сумма", callback_data=f"ec_select:total:{order_id}"
            ),
            InlineKeyboardButton(text="🧾 Расход", callback_data=f"ec_select:materials:{order_id}"),
        )
        kb.row(
            InlineKeyboardButton(
                text="⭐ Отзыв: да", callback_data=f"ec_set_review:yes:{order_id}"
            ),
            InlineKeyboardButton(
                text="⭐ Отзыв: нет", callback_data=f"ec_set_review:no:{order_id}"
            ),
        )
        kb.row(
...
async def admin_edit_closed_select(callback: CallbackQuery, state: FSMContext, user_role: str):
    ...
    if field == "total":
        ...
        await message_obj.edit_text("💰 Введите общую сумму заказа (число, ₽):", reply_markup=None)
    elif field == "materials":
        await state.set_state(EditClosedOrderStates.enter_materials_cost)
        await message_obj.edit_text("🧾 Введите сумму расхода (материалы), ₽:", reply_markup=None)
...
async def admin_edit_closed_set_review(callback: CallbackQuery, state: FSMContext, user_role: str):
    ...
    if isinstance(message_obj, Message):
        await send_edit_closed_menu(message_obj, state)
...
async def admin_edit_closed_save(callback: CallbackQuery, state: FSMContext, user_role: str):
    ...
    try:
        ...
        # Это важно: даже если изменился только has_review или out_of_city, нужно пересчитать прибыль
        master_profit = None
        company_profit = None
        ...
        has_changes = (
            ...
            or has_review is not None
            or out_of_city is not None
        )
        ...
        final_total = (
            updated_order.total_amount or 0.0
            if updated_order
            else (total if total is not None else current.total_amount or 0.0)
        )
        final_materials = (
            updated_order.materials_cost or 0.0
            if updated_order
            else (materials if materials is not None else current.materials_cost or 0.0)
        )
    ...
    if final:
        ...
        await message_obj.edit_text("✅ Изменения сохранены.")
...
async def admin_edit_closed_total(message: Message, state: FSMContext, user_role: str):
    ...
    await state.set_state(EditClosedOrderStates.enter_materials_cost)
    await message.answer(
        "🧾 Введите сумму расхода (материалы), ₽:", reply_markup=get_cancel_keyboard()
    )
...
async def admin_edit_closed_out_of_city(message: Message, state: FSMContext, user_role: str):
    ...
    try:
        ...
        await db.add_audit_log(
            ...
            action="ADMIN_EDIT_CLOSED_ORDER",
            details=(
                f"order_id={order_id}; total={total}; materials={materials}; net={net_profit:.2f}; "
                f"master_profit={master_profit}; company_profit={company_profit}; "
...
async def callback_filter_orders(callback: CallbackQuery, user_role: str):
    ...
    try:
        ...
        if not orders:
            message_obj = callback.message
            if isinstance(message_obj, Message):
                await message_obj.edit_text(f"📭 Нет заявок со статусом '{filter_name}'.")
            await callback.answer()
            return
        ...
        keyboard = get_order_list_keyboard(legacy_orders)
...
async def callback_refuse_order(
    ...
    try:
        ...
        await state.update_data(
            order_id=order_id,
            admin_message_id=message_obj.message_id,
            admin_chat_id=message_obj.chat.id,
...
async def callback_download_masters_excel(callback: CallbackQuery, user_role: str):
    ...
    try:
        ...
        message_obj = callback.message
        if isinstance(message_obj, Message):
            await message_obj.answer_document(document=excel_file, caption="📊 Отчет по мастерам")
...
async def callback_download_statuses_excel(callback: CallbackQuery, user_role: str):
    ...
    try:
        ...
        message_obj = callback.message
        if isinstance(message_obj, Message):
            await message_obj.answer_document(document=excel_file, caption="📊 Отчет по статусам")
...
async def admin_process_out_of_city_confirmation_callback(
    ...
    try:
        ...
        if not order:
            message_obj = callback_query.message
            if isinstance(message_obj, Message):
                await message_obj.edit_text("❌ Заявка не найдена.")
            await state.clear()
            return
        ...
        message_obj = callback_query.message
        if isinstance(message_obj, Message):
            await message_obj.edit_text(summary_message, parse_mode="HTML")
        ...
        message_obj = callback_query.message
        if isinstance(message_obj, Message):
            await message_obj.edit_text(f"❌ Ошибка при закрытии заявки: {e!s}")
    finally:
        await db.disconnect()
