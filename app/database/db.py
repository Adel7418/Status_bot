    ...
    async def get_master_by_telegram_id(self, telegram_id: int) -> Master | None:
        ...
        if row:
            return Master(
                ...
                work_chat_id=row.get("work_chat_id", None),
                created_at=datetime.fromisoformat(row["created_at"]) if row["created_at"] else None,
                username=row["username"],
                first_name=row["first_name"],
                last_name=row["last_name"]
    ...
    async def get_order_by_id(self, order_id: int) -> Order | None:
        ...
        if row:
            return Order(
                ...
                notes=row["notes"],
                total_amount=row.get("total_amount", None),
                materials_cost=row.get("materials_cost", None),
                master_profit=row.get("master_profit", None),
                company_profit=row.get("company_profit", None),
