...
class ReportsNotifier:
        ...
        try:
            ...
            for recipient_id in recipients:
                try:
                    ...
                    logger.error(f"Ошибка отправки ежедневного отчета пользователю {recipient_id}: {e}")
        ...
        try:
            ...
            for recipient_id in recipients:
                try:
                    ...
                    logger.error(f"Ошибка отправки еженедельного отчета пользователю {recipient_id}: {e}")
        ...
        try:
            ...
            for recipient_id in recipients:
                try:
                    ...
                    logger.error(f"Ошибка отправки ежемесячного отчета пользователю {recipient_id}: {e}")
    ...
    async def send_custom_report(self, start_date: str, end_date: str, recipients: List[int] = None):
        ...
        try:
            ...
            file_path = await reports_service.save_report_to_file(report, f"custom_report_{start_date}_{end_date}.txt")
            ...
            for recipient_id in recipients:
                try:
                    ...
                    logger.error(f"Ошибка отправки кастомного отчета пользователю {recipient_id}: {e}")
