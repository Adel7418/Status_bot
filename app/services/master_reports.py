...
class MasterReportsService:
    ...
    async def generate_master_report_excel(
        ...
        if save_to_archive:
            ...
            total_revenue = sum(
                o.total_amount or 0 for o in completed_orders
            )
            
            # Сохраняем запись в БД
            archive_record = MasterReportArchive(
                master_id=master_id,
                period_start=period_start or all_orders[-1].created_at if all_orders else datetime.now(UTC),
                ...
                notes=f"Автоматически созданный отчет за период"
            )
    ...
    async def _fill_active_orders_sheet(self, ws, orders, master):
        ...
        for order in orders:
            ...
            ws.cell(row=row_num, column=2, value=OrderStatus.get_status_name(order.status)).border = border
            ...
            ws.cell(row=row_num, column=8, value=format_datetime(order.created_at) if order.created_at else "").border = border
            row_num += 1
        
        # Итого
        ws.cell(row=row_num + 1, column=1, value="ИТОГО:").font = Font(bold=True)
        ws.cell(row=row_num + 1, column=2, value=f"{len(orders)} активных заявок").font = Font(bold=True)
    ...
    async def _fill_completed_orders_sheet(self, ws, orders, master):
        ...
        for order in orders:
            ...
            ws.cell(row=row_num, column=10, value=format_datetime(order.updated_at) if order.updated_at else "").border = border
        ...
        ws.cell(row=row_num, column=8, value=f"{total_master_profit:.2f} ₽").font = Font(bold=True, color="28a745")
