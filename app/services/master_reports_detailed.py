...
class MasterReportsService:
    ...
    async def _generate_master_report(self, start_date: datetime, report_type: str) -> str | None:
        ...
        try:
            ...
            await self._create_summary_sheet(summary_sheet, orders, period_name, start_date, end_date)
            ...
            for master in masters:
                ...
                if master_orders:  # Создаем лист только если есть заказы
                    ...
                    await self._create_master_sheet(master_sheet, master, master_orders, report_type)
    ...
    async def _get_closed_orders_in_period(self, start_date: datetime, end_date: datetime) -> list:
        ...
        for order in all_orders:
            if (order.status == OrderStatus.CLOSED and 
                order.updated_at):
    ...
    async def _create_summary_sheet(self, ws, orders: list, period_name: str, start_date: datetime = None, end_date: datetime = None):
        ...
        sorted_masters = sorted(master_stats.items(), key=lambda x: x[1]['total_amount'], reverse=True)
    ...
    async def _create_master_sheet(self, ws, master, orders: list, report_type: str = "daily"):
        ...
        if report_type in ["weekly", "monthly"]:
            ...
            headers = ["№ Заказа", "Сумма", "Тип техники", "Сумма к сдаче", "Дата выполнения", "Выезд", "Отзыв"]
