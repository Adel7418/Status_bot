...
class ReportsService:
    ...
    async def generate_weekly_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            closed_orders = await self._get_closed_orders_list(week_start, week_end + timedelta(days=1))
    ...
    async def generate_monthly_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            masters_stats = await self._get_masters_stats(month_start, month_end + timedelta(days=1))
            closed_orders = await self._get_closed_orders_list(month_start, month_end + timedelta(days=1))
            ...
            report = {
                ...
                "orders": orders_stats,
                "masters": masters_stats,
                "summary": await self._get_summary_stats(month_start, month_end + timedelta(days=1)),
                "closed_orders": closed_orders
            }
    ...
    def format_report_to_text(self, report: Dict[str, Any]) -> str:
        ...
        if report_type == "daily":
            title = f"📊 ЕЖЕДНЕВНЫЙ ОТЧЕТ"
            icon = "📅"
        elif report_type == "weekly":
            title = f"📊 ЕЖЕНЕДЕЛЬНЫЙ ОТЧЕТ"
            icon = "📆"
        else:
            title = f"📊 ЕЖЕМЕСЯЧНЫЙ ОТЧЕТ"
            icon = "🗓️"
        ...
        if masters:
            ...
            for i, master in enumerate(masters[:5], 1):  # Топ 5
                ...
                text += f"   Заказов: {master['orders_count']} | Завершено: {master['closed_orders']}\n"
    ...
    async def save_report_to_excel(self, report: Dict[str, Any], filename: str = None) -> str:
        ...
        if closed_orders:
            ...
            row += 1
            headers = ["ID", "Техника", "Клиент", "Мастер", "Сумма", "Материалы", "Прибыль мастера", "Прибыль компании", "Доп. инфо"]
