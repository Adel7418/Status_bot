...
# Примеры: +79001234567, 89001234567, 79001234567, 8(900)123-45-67, 8 (917) 246-23-82
PHONE_PATTERN: Pattern[str] = re.compile(
    r"(?:\+?7|8)[\s\-\(\)]*(?:\d{3})[\s\-\(\)]*\d{3}[\s\-]*\d{2}[\s\-]*\d{2}"
)

# Паттерн для временных индикаторов
# Примеры: 14:00, 10.30, с 9 до 12, к 15:00, до 18, через 1 час, через 1-1.5 часа
TIME_PATTERN: Pattern[str] = re.compile(
    ...
    (?:
        ...
        \b(?:в|В)\s+\d{1,2}\b
        |
        # Через N часа/минут (например: "через 1 час", "через 1-1.5 часа")
        \b(?:через|Через)\s+[\d\-\.]+\s*(?:час|мин|ч|м)
    )
    """,
    re.VERBOSE,
)
...
# ВАЖНО: Все слова должны быть в нижнем регистре (проверка идет через text.lower())
...
def contains_time_indicator(text: str) -> bool:
    ...
    strong_indicators = ["сегодня", "завтра", "послезавтра", "утром", "днём", "днем", "вечером", "ночью", "срочно", "асап", "время", "часов", "через"]
...
def looks_like_address(text: str) -> bool:  # noqa: PLR0911
    ...
    time_keywords_strong = ["завтра", "сегодня", "послезавтра", "утром", "днём", "днем", "вечером", "ночью", "срочно", "через"]
    ...
    if has_digits and has_letters:
        ...
        # Если строка не слишком длинная (< 60 символов) и содержит цифры с буквами
        # скорее всего это адрес - но только если нет явных признаков проблемы
        # Исключаем строки с типичными словами проблем (используем границы слов \b)
        problem_indicators = ["не", "течёт", "течет", "шумит", "крутит", "работает", "включается", "выключается"]
        has_problem_word = any(re.search(r'\b' + re.escape(word) + r'\b', text_lower) for word in problem_indicators)
