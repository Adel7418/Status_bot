...
async def apply_migrations_manually():
    ...
    try:
        ...
        if 'estimated_completion_date' not in columns:
            await db.connection.execute("ALTER TABLE orders ADD COLUMN estimated_completion_date TEXT")
        ...
        await db.connection.execute("CREATE TABLE IF NOT EXISTS alembic_version (version_num TEXT NOT NULL)")
        await db.connection.execute("DELETE FROM alembic_version")
        await db.connection.execute("INSERT INTO alembic_version (version_num) VALUES (?)", ('004_add_order_reports',))
