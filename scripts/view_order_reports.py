...
def view_order_reports(db_path: str, start_date: str = None, end_date: str = None):
    ...
    # Заголовки
    print(f"{'ID':<4} {'Заказ':<6} {'Техника':<20} {'Клиент':<20} {'Мастер':<20} {'Сумма':<10} {'Мастер':<10} {'Компания':<10} {'Выезд':<6} {'Отзыв':<6} {'Время':<8}")
    print("-" * 140)
    ...
    for row in rows:
        ...
        print(f"{row[0]:<4} #{order_id:<5} {equipment:<20} {client:<20} {master:<20} {amount:<10.2f} {master_profit:<10.2f} {company_profit:<10.2f} {out_of_city:<6} {has_review:<6} {completion_time:<8}")
    ...
    print("-" * 140)
    print(f"{'ИТОГО:':<4} {'':<5} {'':<20} {'':<20} {'':<20} {total_amount:<10.2f} {total_master_profit:<10.2f} {total_company_profit:<10.2f} {out_of_city_count:<6} {review_count:<6} {'':<8}")
    ...
    if len(rows) > 0:
        ...
        avg_completion = sum(row[17] for row in rows if row[17]) / len([row for row in rows if row[17]])
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Использование: python view_order_reports.py [путь_к_базе] [начальная_дата] [конечная_дата]")
